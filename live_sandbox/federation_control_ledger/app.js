...
const LAYERS = [
  { id:'core', title:'Core', color:'#00ffff', match:n=>n.id.includes('data-federation-map') || n.id==='repo-root', isRepo:true },
  { id:'data', title:'Data repos', color:'#00e5ff', match:n=>n.repo_type==='data' && !n.id.includes('data-federation-map') && n.id!=='repo-root', isRepo:true },
...
const PAGE_LINKS = {
  ...
  'live-dashboard-page':'https://ventusltd.github.io/data-federation-map-for-globalgrid2050-all-repos/live_sandbox/federation_control_ledger/'
};
const EDGE_HELP = {
  data:'Data dependency: one object consumes or publishes cartridge data used by another object.',
  governance:'Governance relationship: documentation, report, doctrine or control dependency.',
  archive:'Archive lineage: relationship to the original source archive or retiring monolith.',
  external:'External reference: dependency on a public host, build system, API or workflow runner.',
  repo:'Repository structure: declared parent-child or file relationship.',
  workflow:'Dependency on an external data source, build runner or processing tool.'
};
let state = { filter:'all', selected:null, selectedEdge:null, scale:defaultScale(), visibleLayers:new Set(LAYERS.filter(l=>l.isRepo).map(l=>l.id)), nodes:[], edges:[], manifest:null, manifestUrl:null, baseUrl:null, cards:new Map(), dims:null };
let scopeStack = [];
let pinch = null;
...
function defaultVisible(){return new Set(LAYERS.filter(l=>l.isRepo).map(l=>l.id));}
...
async function loadScope(manifestPath, push){
  document.body.classList.add('focus-mode');
  const baseForResolve = state.baseUrl || location.href;
  const manifestUrl = rootUrl(manifestPath, baseForResolve);
  const manifest = await json(manifestUrl);
  if(push && state.manifestUrl) scopeStack.push(state.manifestUrl);
  ...
  state.visibleLayers = defaultVisible();
  ...
  render();
  updateBackButton();
  document.querySelector('.board-wrap')?.scrollTo({left:0,top:0,behavior:'smooth'});
}

function grouped(){return LAYERS.map(layer=>({ ...layer, items:state.nodes.filter(n=>n.layerId===layer.id && state.visibleLayers.has(layer.id)).sort((a,b)=>b.importance-a.importance) })).filter(g=>g.items.length);}
...
function connectors(){
  ...
  visibleEdges().forEach(e=>{
    ...
    const p=document.createElementNS('http://www.w3.org/2000/svg','path'); p.setAttribute('d',c.path); p.setAttribute('class',`connector-path ${typeClass}${isDim?' dim':''}${isHighlight?' highlight':''}`); svg.appendChild(p);
    const hit=document.createElementNS('http://www.w3.org/2000/svg','path'); hit.setAttribute('d',c.path); hit.setAttribute('class','connector-hit'); hit.addEventListener('click',ev=>{ev.stopPropagation(); state.selected=null; state.selectedEdge=e.id; showEdgePopover(e,c.labelX,c.labelY); redrawOnly();}); svg.appendChild(hit);
  });
  return svg;
}
...
function showEdgePopover(e,x,y){
  ...
  const pop=document.createElement('div'); pop.className='edge-popover'; pop.style.left=`${x}px`; pop.style.top=`${Math.max(10,y-54)}px`;
...
function card(n){
  ...
  const childLink=div.querySelector('[data-child-card]'); if(childLink) childLink.addEventListener('click',ev=>{ev.preventDefault(); ev.stopPropagation(); loadScope(n.child_manifest,true);});
  return div;
}
function repoPreview(n){
  ...
  const workflows = String(n.id||'').startsWith('Ventusltd/') ? `https://github.com/${n.id}/actions` : '';
  ...
  const open=pop.querySelector('[data-open-child]'); if(open) open.onclick=e=>{e.preventDefault(); loadScope(n.child_manifest,true);};
  return pop;
}
function buildLayerKey(){
  ...
  nav.querySelector('[data-mode="repos"]').onclick=()=>{state.visibleLayers=defaultVisible(); state.filter='all'; state.selected=null; state.selectedEdge=null; render();};
...
function updateLayerKeyState(){const nav=document.getElementById('layerNav'); if(!nav) return; nav.querySelectorAll('[data-layer]').forEach(btn=>btn.classList.toggle('active',state.visibleLayers.has(btn.dataset.layer)));}
function ensureBackButton(){let btn=document.getElementById('backButton'); if(btn) return btn; const actions=document.querySelector('.actions'); btn=document.createElement('button'); btn.id='backButton'; btn.type='button'; btn.textContent='Back'; actions.prepend(btn); btn.onclick=()=>{const parent=scopeStack.pop(); if(parent){state.baseUrl=new URL('.', parent).href; loadScope(parent,false);}}; return btn;}
function updateBackButton(){const btn=ensureBackButton(); btn.hidden=scopeStack.length===0;}
function setScale(value){state.scale=Math.max(0.42,Math.min(1.25,value)); render();}
function wire(){
  ensureBackButton(); updateBackButton();
  ...
  document.getElementById('resetButton').onclick=()=>{state.filter='all';state.selected=null;state.selectedEdge=null;state.visibleLayers=defaultVisible();setScale(defaultScale());document.querySelector('.board-wrap').scrollTo({left:0,top:0,behavior:'smooth'});};
...
wire();
loadScope(ROOT,false).catch(err=>{document.getElementById('board').innerHTML=`<p style="padding:16px;color:#ff6666">${esc(err.message)}</p>`;});
